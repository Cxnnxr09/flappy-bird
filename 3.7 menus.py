...
class Bird:
    ...
    def get_coords(self):
        bird_position = self._canvas.coords(
            self._rectangle)  # Sets new bird coords
...
class Obstacle:
    ...
    def move_obstacle(self):
        ...
        self.top_obstacle_position = self._canvas.coords(self._top_rectangle)
        self.bottom_obstacle_position = self._canvas.coords(
            self._bottom_rectangle)
...
class Scoreboard:
    ...
    def __init__(self, x, y, fontsize, canvas):
        self.x = x
        self.y = y
        self.score = 0
        self._canvas = canvas
        self.fontsize = fontsize
        self.text = Label(self._canvas, fg="black", text=self.score,
                          font=("Arial", fontsize))
...
class Game:
    ...
    def __init__(self, canvas):
        ...
        self.obstacles = []
        self.scoreboard = Scoreboard(0, 0, 30, canvas)
    ...
    def manage_pipes(self):
        ...
        if len(self.obstacles) == 0:
            obstacle = Obstacle(self.canvas, SCREEN_WIDTH,
                                random.randrange(150, 600))
            self.obstacles.append(obstacle)
        for obstacle in self.obstacles:
            if obstacle.halfway:
                new_obstacle = Obstacle(self.canvas, SCREEN_WIDTH +
...
class Menu:
    ...
    def __init__(self, window):
        self.menu_frame = Frame(window, width=360, height=640)
        self.menu_frame.pack()
        self.label = Label(self.menu_frame, text="Flappy Bird",
                           font=("arial", 35, "bold"), justify="center")
        self.label.pack()
        self.play_button = Button(self.menu_frame, text="Play",
                                  font=("arial", 25, "bold"),
                                  command=lambda: self.change_frame(),
                                  width=self.BUTTON_WIDTH,
                                  height=self.BUTTON_HEIGHT)
        self.play_button.pack()
        self.exit_button = Button(self.menu_frame, text="Exit!",
                                  font=("arial", 25, "bold"),
                                  command=lambda: exit(),
                                  width=self.BUTTON_WIDTH,
                                  height=self.BUTTON_HEIGHT)
        self.exit_button.pack()
        self.game_canvas = Canvas(window, width=360, height=640,
                                  background="skyblue")

    def change_frame(self):
        self.menu_frame.pack_forget()
        self.game_canvas.pack()
        Game_object = Game(self.game_canvas)
